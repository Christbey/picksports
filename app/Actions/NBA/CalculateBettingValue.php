...
class CalculateBettingValue
{
    ...
    protected function analyzeSpread(Game $game, $prediction, array $market): ?array
    {
        ...
        // Edge threshold for recommendation (lower for NBA since lines are tighter)
        if ($edge < config('nba.betting.edge_thresholds.spread')) {
            return null;
        }
    ...
    protected function analyzeTotal(object $prediction, array $market): ?array
    {
        ...
        // Edge threshold for totals (lower for NBA since lines are tighter)
        if ($edge < config('nba.betting.edge_thresholds.total')) {
            return null;
        }
    ...
    protected function analyzeMoneyline(Game $game, object $prediction, array $market): ?array
    {
        ...
        $minEdge = config('nba.betting.edge_thresholds.moneyline');
        ...
        $kellyConfig = config('nba.betting.kelly');
    ...
    {
        // NBA teams use school (city) + mascot for display
        $city = $team->school ?? '';
        $mascot = $team->mascot ?? '';
    ...
    protected function teamMatchesOutcome($team, string $outcomeName, string $oddsApiTeamName): bool
    {
        ...
        // Check if team city is contained in the outcome name

...
class SyncOddsForGames
{
    ...
    protected function fuzzyMatchTeams(Game $game, array $event): bool
    {
        ...
        // CFB teams use school names
        $espnHomeName = $game->homeTeam->name ?? '';
        $espnAwayName = $game->awayTeam->name ?? '';
        $espnHomeLocation = $game->homeTeam->location ?? '';
        $espnAwayLocation = $game->awayTeam->location ?? '';
        $espnHomeDisplay = $game->homeTeam->display_name ?? '';
        $espnAwayDisplay = $game->awayTeam->display_name ?? '';
        $espnHomeAbbr = $game->homeTeam->abbreviation ?? '';
        $espnAwayAbbr = $game->awayTeam->abbreviation ?? '';

        $homeNormalized = $this->normalizeTeamName($espnHomeLocation);
        $awayNormalized = $this->normalizeTeamName($espnAwayLocation);

...
class GradePredictions
{
    public function execute(?int $season = null): array
    {
        $query = Prediction::query()
            ->join('cfb_games', 'cfb_predictions.game_id', '=', 'cfb_games.id')
            ->where('cfb_games.status', 'STATUS_FINAL')
            ->whereNotNull('cfb_games.home_score')
            ->whereNotNull('cfb_games.away_score')
            ->whereNull('cfb_predictions.graded_at')
            ->select('cfb_predictions.*', 'cfb_games.home_score', 'cfb_games.away_score');
    ...
    public function getStatsByConfidence(?int $season = null): Collection
    {
        ...
        if ($season) {
            $query->join('cfb_games', 'cfb_predictions.game_id', '=', 'cfb_games.id')
                ->where('cfb_games.season', $season)
                ->select('cfb_predictions.*');
        }

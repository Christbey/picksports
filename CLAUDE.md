...
- `calculation-regression-investigator` — Activate when a sports metric, Elo value, prediction, betting value, live update, injury adjustment, or recommendation looks wrong, drifts unexpectedly, or needs root-cause analysis. Use it to trace the exact output back through resources, services, sport-specific actions, shared abstractions, input filters, and scheduler timing before changing formulas.
- `sports-contract-reviewer` — Activate when changing shared or sport-specific sports data contracts, including API routes, controllers, resources, frontend game-page consumers, prediction payloads, team metrics, season or season_type handling, scheduler-driven sports pipelines, or sports-domain semantics. Use it to check shared abstractions first, trace frontend consumers, verify sport-specific exceptions, and confirm the relevant contract or calculation tests still hold.
- `scheduler-operator` — Activate when working on scheduled sports pipelines, season-gated jobs, queue-backed sync workflows, alert digests, operational timing issues, or scheduler documentation. Use it to trace real schedule definitions in `routes/console.php`, confirm upstream/downstream job ordering, distinguish queue issues from schedule issues, and keep `SCHEDULER_REFERENCE.md` aligned with code.

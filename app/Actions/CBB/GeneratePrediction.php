...
class GeneratePrediction extends AbstractPredictionGenerator
{
    ...
    private function getRecentForm(Team $team, int $season): array
    {
        ...
        $stats = TeamStat::query()
            ...
            ->join('cbb_games', 'cbb_team_stats.game_id', '=', 'cbb_games.id')
            ->orderByDesc('cbb_games.game_date')
            ->select('cbb_team_stats.*')
            ->get();
    ...
    {
        $defaultEff = config('cbb.prediction.default_efficiency');

...
class FetchTeamDepthCharts implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->teamEspnId) {
            ...
            Log::info("NBA: Synced {$count} depth chart entries for team {$this->teamEspnId} in season {$season} from ESPN");

            return;
        }

        $count = $action->syncAllTeams($season);
        Log::info("NBA: Synced {$count} total depth chart entries for season {$season} from ESPN");
    }
}

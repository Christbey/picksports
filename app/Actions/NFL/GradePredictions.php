...
class GradePredictions
{
    public function execute(?int $season = null): array
    {
        $query = Prediction::query()
            ->join('nfl_games', 'nfl_predictions.game_id', '=', 'nfl_games.id')
            ->where('nfl_games.status', 'STATUS_FINAL')
            ->whereNotNull('nfl_games.home_score')
            ->whereNotNull('nfl_games.away_score')
            ->whereNull('nfl_predictions.graded_at')
            ->select('nfl_predictions.*', 'nfl_games.home_score', 'nfl_games.away_score');
    ...
    public function getStatsByConfidence(?int $season = null): Collection
    {
        ...
        if ($season) {
            $query->join('nfl_games', 'nfl_predictions.game_id', '=', 'nfl_games.id')

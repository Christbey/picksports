...
it('respects minimum games threshold', function () {
    $minimumGames = config('cbb.metrics.minimum_games');
    ...
    expect($metric->offensive_efficiency)->toBeNull();
    expect($metric->defensive_efficiency)->toBeNull();
    expect($metric->net_rating)->toBeNull();
    expect($metric->tempo)->toBeNull();
});

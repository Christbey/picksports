...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    { title: 'WNBA', href: '/wnba-predictions' },
    { title: 'Games', href: '/wnba-predictions' },
    { title: `Game ${props.game.id}`, href: `/wnba/games/${props.game.id}` }
])
...
onMounted(async () => {
    try {
        ...
        const [gameRes, predictionRes] = await Promise.all([
            fetch(`/api/v1/wnba/games/${props.game.id}`),
            fetch(`/api/v1/wnba/games/${props.game.id}/prediction`)
        ])
        ...
        if (gameRes.ok) {
            ...
            if (fullGame.home_team?.id) {
                const homeMetricsRes = await fetch(`/api/v1/wnba/teams/${fullGame.home_team.id}/metrics`)
            ...
            if (fullGame.away_team?.id) {
                const awayMetricsRes = await fetch(`/api/v1/wnba/teams/${fullGame.away_team.id}/metrics`)
                ...
                <div class="rounded-xl overflow-hidden bg-gradient-to-r from-purple-600 to-purple-800 dark:from-purple-800 dark:to-purple-950 text-white shadow-lg">
                    <div class="px-6 py-8">
                        <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                            <Link
                                v-if="awayTeam"
                                :href="WNBATeamController(awayTeam.id)"
                            ...
                            <Link
                                v-if="homeTeam"
                                :href="WNBATeamController(homeTeam.id)"

...
class CalculateElo
{
    public function execute(Game $game, bool $skipIfExists = true): array
    {
        ...
        $homeElo = $homeTeam->elo_rating ?? config('cfb.elo.default_rating');
        $awayElo = $awayTeam->elo_rating ?? config('cfb.elo.default_rating');

        // Adjust for home field advantage (skip if neutral site)
        $homeFieldAdvantage = $game->neutral_site ? 0 : config('cfb.elo.home_field_advantage');
        $adjustedHomeElo = $homeElo + $homeFieldAdvantage;

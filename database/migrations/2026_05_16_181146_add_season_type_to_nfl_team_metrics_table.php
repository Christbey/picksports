...
{
    private const TABLE = 'nfl_team_metrics';

    private const LEGACY_UNIQUE = 'nfl_team_metrics_team_id_season_unique';

    private const TEAM_ID_INDEX = 'nfl_team_metrics_team_id_idx';

    private const SEASON_TYPE_UNIQUE = 'nfl_team_metrics_team_id_season_season_type_unique';
    ...
    public function up(): void
    {
        ...
        DB::table(self::TABLE)
            ...
            ->update(['season_type' => (string) config('nfl.season.types.regular', 2)]);

        Schema::table(self::TABLE, function (Blueprint $table) {
            $table->string('season_type')
                ->default((string) config('nfl.season.types.regular', 2))
                ->nullable(false)
                ->change();
        });

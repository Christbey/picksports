...
class GradePredictions
{
    public function execute(?int $season = null): array
    {
        $query = Prediction::query()
            ->join('wcbb_games', 'wcbb_predictions.game_id', '=', 'wcbb_games.id')
            ->where('wcbb_games.status', 'STATUS_FINAL')
            ->whereNotNull('wcbb_games.home_score')
            ->whereNotNull('wcbb_games.away_score')
            ->whereNull('wcbb_predictions.graded_at')
            ->select('wcbb_predictions.*', 'wcbb_games.home_score', 'wcbb_games.away_score');

        if ($season) {
            $query->where('wcbb_games.season', $season);
        }
    ...
    public function getStatsByConfidence(?int $season = null): Collection
    {
        ...
        if ($season) {
            $query->join('wcbb_games', 'wcbb_predictions.game_id', '=', 'wcbb_games.id')
                ->where('wcbb_games.season', $season)
                ->select('wcbb_predictions.*');
        }

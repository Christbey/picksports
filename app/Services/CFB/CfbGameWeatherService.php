...
class CfbGameWeatherService
{
     ...
     */
    private function resolveLocation(Game $game): ?array
    {
        $coordinates = (array) config('cfb.predictions.game_context.weather.venue_coordinates', []);
        $keys = array_filter([
     ...
     */
    private function nearestHourlyIndex(array $times, Carbon $target): int
    {
        $bestIndex = 0;
     ...
     */
    private function hourlyValue(array $payload, string $key, int $index): mixed
    {
        return data_get($payload, "hourly.{$key}.{$index}");
    }

    private function isIndoorVenue(Game $game): bool
    {

...
- Material source documents, availability/player-linkage, synced injury details, team/QB/coach/venue/kickoff changes and coarse weather/roof changes invalidate reuse. Document ordering, source-check timestamps and market/model-only changes do not cause paid research.
...
- Numerical projections and market revisions continue locally. Changed model candidates retain `research_candidate_changed`: old pro/con arguments are not presented as newly verified against the changed forecast. Expired, changed, partial or deferred research remains on hold, including prop/market holds. Prediction payloads reject changed fingerprinted evidence and cannot fall back past a newer insufficient report to an older ready one.

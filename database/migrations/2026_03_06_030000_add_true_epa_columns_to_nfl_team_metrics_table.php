...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('nfl_team_metrics', function (Blueprint $table) {
            if (! Schema::hasColumn('nfl_team_metrics', 'offensive_true_epa_per_play')) {
                $table->decimal('offensive_true_epa_per_play', 8, 3)
                    ->nullable()
                    ->after('second_half_rating');
            }

            if (! Schema::hasColumn('nfl_team_metrics', 'defensive_true_epa_per_play')) {
                $table->decimal('defensive_true_epa_per_play', 8, 3)
                    ->nullable()
                    ->after('offensive_true_epa_per_play');
            }

            if (! Schema::hasColumn('nfl_team_metrics', 'net_true_epa_per_play')) {

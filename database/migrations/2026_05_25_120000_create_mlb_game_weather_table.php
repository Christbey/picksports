...
return new class extends Migration
{
    ...
    {
        Schema::create('mlb_game_weather', function (Blueprint $table) {
            $table->id();
            $table->foreignId('game_id')->unique()->constrained('mlb_games')->cascadeOnDelete();
            ...
            $table->string('roof_status', 32)->nullable();
            $table->json('raw_payload')->nullable();
            $table->timestamps();

            $table->index(['provider', 'observed_at']);
            $table->index(['is_indoor', 'wind_speed_mph']);
            $table->index('roof_status');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('mlb_game_weather');
    }
};

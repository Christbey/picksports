...
{
    protected $signature = 'cfb:sync-game-weather
        {--season= : CFB season to sync}
    ...
    protected $description = 'Sync kickoff weather for CFB games';

    public function handle(CfbGameWeatherService $weatherService): int
    {
        $query = Game::query()
        ...
        $games = $query->get();
        if ($games->isEmpty()) {
            $this->warn('No CFB games matched the weather sync scope.');

...
{
    protected $signature = 'nfl:grade-player-props
                            {--season= : Grade player props for a specific season (defaults to current year)}';

    protected $description = 'Grade NFL player props against actual player statistics';
    ...
    public function handle(): int
    {
        ...
        $this->info("Grading NFL player props for season {$season}...");
        $this->newLine();

        $results = $gradePlayerProps->execute('americanfootball_nfl', $season);
        ...
        $statsByMarket = $gradePlayerProps->getStatsByMarket('americanfootball_nfl', $season);

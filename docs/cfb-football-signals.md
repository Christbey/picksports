...
**Registration is not validation.** These are research hypotheses with predefined cohort boundaries, not 200 proven betting edges. Conditions have no hand-assigned prediction coefficient. The residual learner must support a condition using eligible frozen historical forecasts and chronological held-out validation before it can alter a forecast. Family averaging and caps limit overlapping conditions.

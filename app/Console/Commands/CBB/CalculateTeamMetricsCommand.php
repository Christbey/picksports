...
class CalculateTeamMetricsCommand extends Command
{
    ...
    protected $signature = 'cbb:calculate-team-metrics
                            {--season= : Calculate metrics for a specific season (defaults to current year)}
                            {--team= : Calculate metrics for a specific team ID}';

    protected $description = 'Calculate CBB team advanced metrics (offensive/defensive efficiency, tempo, net rating)';
    ...
    protected function displayTeamMetric(TeamMetric $metric): void
    {
        ...
        $this->newLine();
        $this->info('Rolling Metrics (Last '.config('cbb.metrics.rolling_window_size').' Games):');
        $this->table(
            ['Metric', 'Value'],
            [

...
const regionRoundKeys = ['round_of_64', 'round_of_32', 'sweet_16', 'elite_8'] as const
...
const participantKey = (participant: BracketParticipant | null) => participant?.id ?? null
...
const participantWithSeed = (team: Team, seed: number | null): BracketParticipant => ({
  ...teamToParticipant(team),
  seed,
})

const participantLogo = (slot: BracketSlot | null) => resolveSlotParticipant(slot)?.logo ?? null
const bracketSeason = computed(() => {
  const season = props.regions?.flatMap((region) => region.rounds.flatMap((round) => round.games.map((game) => Number(game.gameDate?.slice(0, 4))))).find(Boolean)
  return season || new Date().getFullYear()
})

const fallbackParticipantForSeed = (regionName: string, seed: number): BracketParticipant | null => {
...
const formatVenue = (game: TournamentGame | null) => {
  ...
  const parts = [game.venueName, game.venueCity, game.venueState].filter(Boolean)
...
const isRoundOf64PlayInCard = (matchup: BracketMatchup) =>
  matchup.roundKey === 'round_of_64' && matchup.participants.some((slot) => slot.sourceMatchupId)

const sortedGames = (games: TournamentGame[]) =>
  [...games].sort((left, right) => `${left.gameDate ?? ''} ${left.gameTime ?? ''}`.localeCompare(`${right.gameDate ?? ''} ${right.gameTime ?? ''}`))

const allTournamentGames = computed(() => (props.regions ?? []).flatMap((region) => region.rounds.flatMap((round) => round.games)))
const firstFourMatchups = computed<BracketMatchup[]>(() =>
  sortedGames(allTournamentGames.value.filter((game) => game.roundKey === 'first_four')).map((game) => ({
    id: `game:${game.id}`,
    roundKey: 'first_four',
...
const firstFourMatchupsById = computed(() => new Map(firstFourMatchups.value.map((matchup) => [matchup.id, matchup])))
...
const sourceMatchupLabel = (slot: BracketSlot | null) => {
  ...
  const participants = sourceMatchup.participants
    ...
    .filter((participant): participant is BracketParticipant => Boolean(participant))
...
const participantButtonAbbr = (slot: BracketSlot | null, fallback: string) =>
  resolveSlotParticipant(slot)?.abbreviation ?? slot?.placeholderAbbreviation ?? fallback
const participantSeed = (slot: BracketSlot | null) => resolveSlotParticipant(slot)?.seed ?? null

const slotHasResolvedParticipant = (slot: BracketSlot | null) => Boolean(resolveSlotParticipant(slot))
const slotIsPlaceholder = (slot: BracketSlot | null) => !slotHasResolvedParticipant(slot) && !sourceMatchupLabel(slot)

const slotWinProbability = (matchup: BracketMatchup, slot: BracketSlot | null) => {
  const participant = resolveSlotParticipant(slot)
...
const slotWinProbabilityClass = (matchup: BracketMatchup, slot: BracketSlot | null) => {
...
const matchupResult = (matchup: BracketMatchup) => currentBracket.value?.results?.[matchup.id] ?? null

const selectedSlotResultStatus = (matchup: BracketMatchup, slot: BracketSlot | null) => {
  const participant = resolveSlotParticipant(slot)
...
const resultBadgeLabel = (matchup: BracketMatchup, slot: BracketSlot | null) => {
  const status = selectedSlotResultStatus(matchup, slot)
...
const resultBadgeClass = (matchup: BracketMatchup, slot: BracketSlot | null) => {
  const status = selectedSlotResultStatus(matchup, slot)

  if (status === 'correct') return 'border-emerald-300/20 bg-emerald-300/12 text-emerald-100/85'
  if (status === 'incorrect') return 'border-rose-300/20 bg-rose-300/12 text-rose-100/85'
  if (status === 'pending') return 'border-amber-300/20 bg-amber-300/12 text-amber-100/85'
...
const matchupOpponentSummary = (matchup: BracketMatchup) => {
  const playInSlot = matchup.participants.find((slot) => slot.sourceMatchupId)
  if (!playInSlot) return null

  const opposingSlot = matchup.participants.find((slot) => slot !== playInSlot) ?? null
...
const regionMap = computed(() => {
  const map = new Map((props.regions ?? []).map((region) => [region.name, region]))
...
const buildVirtualRound = (
  ...
  const matchupCount = actualGames.length > 0 ? actualGames.length : Math.ceil(previousMatchups.length / 2)

  return Array.from({ length: matchupCount }, (_, index) => {
    const leftWinner = selectedParticipant(previousMatchups[index * 2] ?? null)
    const rightWinner = selectedParticipant(previousMatchups[(index * 2) + 1] ?? null)
...
const buildRegionBracket = (region: TournamentRegion, firstFourMatchups: BracketMatchup[]): BracketRegion => {
  const roundMap = new Map(region.rounds.map((round) => [round.key, sortedGames(round.games)]))
  const roundOf64Games = roundMap.get('round_of_64') ?? []

  const regionFirstFour = firstFourMatchups.filter((matchup) => matchup.game?.region === region.name)
  const roundOf64GamesByPair = new Map(
    roundOf64Games
      .filter((game) => game.homeSeed != null && game.awaySeed != null)
      .map((game) => [`${Math.min(game.homeSeed!, game.awaySeed!)}-${Math.max(game.homeSeed!, game.awaySeed!)}`, game]),
  )

  const roundOf64Matchups: BracketMatchup[] = roundOf64SeedPairings.map(([highSeed, lowSeed], index) => {
    const game = roundOf64GamesByPair.get(`${Math.min(highSeed, lowSeed)}-${Math.max(highSeed, lowSeed)}`) ?? null
    const matchingFirstFour = regionFirstFour.find((matchup) => matchup.game?.playInTargetSeed === highSeed || matchup.game?.playInTargetSeed === lowSeed)
    const playInSeed = matchingFirstFour?.game?.playInTargetSeed

    const slotForSeed = (seed: number, side: 'home' | 'away'): BracketSlot => {
      if (playInSeed === seed && matchingFirstFour) {
        return firstFourWinnerSlot(matchingFirstFour)
      }

      if (!game) {
        const fallbackParticipant = fallbackParticipantForSeed(region.name, seed)
        return fallbackParticipant ? { participant: fallbackParticipant } : seedPlaceholderSlot(seed)
      }

      const team = side === 'home' ? game.homeTeam : game.awayTeam
      const teamSeed = side === 'home' ? game.homeSeed : game.awaySeed
      ...
      const oppositeSeed = side === 'home' ? game.awaySeed : game.homeSeed

      if (oppositeSeed === seed) {
        return { participant: participantWithSeed(oppositeTeam, oppositeSeed) }
      }
    ...
    return {
      id: game ? `game:${game.id}` : `${region.name}-round_of_64-${index}`,
  ...
  const roundOf32Matchups = buildVirtualRound(region.name, 'round_of_32', roundOf64Matchups, roundMap.get('round_of_32') ?? [])
  const sweet16Matchups = buildVirtualRound(region.name, 'sweet_16', roundOf32Matchups, roundMap.get('sweet_16') ?? [])
  const elite8Matchups = buildVirtualRound(region.name, 'elite_8', sweet16Matchups, roundMap.get('elite_8') ?? [])

  return {
    id: region.id,
    name: region.name,
    rounds: [
      { key: 'round_of_64', label: roundLabels.round_of_64, matchups: roundOf64Matchups },
      { key: 'round_of_32', label: roundLabels.round_of_32, matchups: roundOf32Matchups },
      { key: 'sweet_16', label: roundLabels.sweet_16, matchups: sweet16Matchups },
      { key: 'elite_8', label: roundLabels.elite_8, matchups: elite8Matchups },
    ],
  }
}
...
const roundTrackConfig: Record<string, { minHeight: string; gridRows: string; starts: number[]; span: number }> = {
  round_of_64: {
...
const roundColumnMinHeightClass = (roundKey: string) => roundTrackConfig[roundKey]?.minHeight ?? 'min-h-[22rem]'
...
const roundColumnGridClass = (roundKey: string) => roundTrackConfig[roundKey]?.gridRows ?? 'grid-rows-[repeat(12,minmax(0,1rem))]'

const allRegionMatchups = computed(() => bracketRegions.value.flatMap((region) => region.rounds.flatMap((round) => round.matchups)))

const standaloneRounds = computed(() => {
  const finalFourGames = sortedGames(allTournamentGames.value.filter((game) => game.roundKey === 'final_four'))
  const championshipGames = sortedGames(allTournamentGames.value.filter((game) => game.roundKey === 'national_championship'))

  const regionChampion = (regionName: string) => {
    const region = bracketRegions.value.find((entry) => entry.name === regionName)
    const elite8Round = region?.rounds.find((round) => round.key === 'elite_8')
    return elite8Round?.matchups[0] ? selectedParticipant(elite8Round.matchups[0]) : null
  }
  ...
  const championshipMatchup: BracketMatchup = {
    ...
    participants: [
      { participant: selectedParticipant(finalFourMatchups[0]), placeholderLabel: 'Semifinal winner', placeholderAbbreviation: 'SF1' },
      { participant: selectedParticipant(finalFourMatchups[1]), placeholderLabel: 'Semifinal winner', placeholderAbbreviation: 'SF2' },
    ],
  }
...
const completionPct = computed(() => (totalGames.value === 0 ? 0 : Math.round((pickedGames.value / totalGames.value) * 100)))
const champion = computed(() => selectedParticipant(standaloneRounds.value.championship)?.name ?? null)
const activeBracketName = computed(() => currentBracket.value?.name?.trim() || 'Untitled bracket')
...
const roundState = (region: BracketRegion, roundKey: string) => {
  ...
  const activeIndex = region.rounds.findIndex((round) => round.key === activeRound)
  const roundIndex = region.rounds.findIndex((round) => round.key === roundKey)
...
const upsertBracketInList = (bracket: SavedBracket) => {
  ...
  const index = next.findIndex((entry) => entry.public_id === bracket.public_id)
  ...
  brackets.value = next.sort((left, right) => new Date(right.updated_at ?? 0).getTime() - new Date(left.updated_at ?? 0).getTime())
}
...
const applyStoredPicks = (stored: Record<string, string>) => {
  const validMatchupIds = new Set(allBracketMatchups.value.map((matchup) => matchup.id))
  picks.value = Object.fromEntries(Object.entries(stored).filter(([matchupId]) => validMatchupIds.has(matchupId)))
}
...
const createBracket = async (initialPicks: Record<string, string> = {}, syncPicks = true) => {
...
const saveBracketMeta = async () => {
  if (!isAuthenticated.value || !currentBracket.value || isBracketLocked.value) {
    return
  }
...
const switchBracket = (publicId: string) => {
  const bracket = brackets.value.find((entry) => entry.public_id === publicId) ?? null
...
const loadServerBrackets = async () => {
  ...
  try {
    ...
    if (serverBrackets.length > 0) {
      const activeBracket = serverBrackets.find((entry) => entry.public_id === activeBracketPublicId.value) ?? serverBrackets[0]
      activateBracket(activeBracket)
    } else if (!isBracketLocked.value && Object.keys(picks.value).length > 0) {
...
watch(
  ...
  (value, _oldValue, onCleanup) => {
    if (!isAuthenticated.value || !storageLoaded.value || !serverLoaded.value || isBracketLocked.value) {
      return
    }
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        ...
        if (activeBracketPublicId.value) {
          const response = await axios.patch(`/api/v1/cbb-brackets/${activeBracketPublicId.value}`, {
            picks: value,
          })
        ...
        if (error?.response?.status === 423) {
          ...
          currentBracket.value = currentBracket.value
            ? {
                ...
                is_locked: true,
                can_edit: false,
                lock_at: error?.response?.data?.lock_at ?? currentBracket.value.lock_at ?? bracketLockAtIso,
              }
            : null
        } else {
...
watch(
  ...
  (matchups) => {
    ...
    const filteredEntries = Object.entries(picks.value).filter(([matchupId, participantId]) => validSelections.get(matchupId)?.has(participantId))
...
<template>
  <Head title="March Madness Bracket Builder">
    <meta head-key="description" name="description" content="Build a March Madness bracket using real NCAA tournament games synced into PickSports." />
    <meta head-key="og:title" property="og:title" content="March Madness Bracket Builder" />
    <meta head-key="og:description" property="og:description" content="Interactive March Madness bracket builder powered by real tournament game data." />
  </Head>
  ...
  <div class="min-h-screen overflow-x-hidden bg-background text-foreground">
    ...
    >
      <div class="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" class="flex items-center gap-3">
          <div class="flex size-9 items-center justify-center rounded-xl bg-primary text-sm font-black text-primary-foreground">
            PS
          </div>
          <div>
            <p class="text-sm font-semibold uppercase tracking-[0.24em] text-foreground/90">PickSports</p>
            <p class="text-xs text-muted-foreground">Bracket Builder</p>
          </div>
        </Link>
    ...
    >
          ...
          <div class="ui-surface rounded-[2rem] p-4 sm:p-5">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <div class="rounded-2xl border border-border/70 bg-card/55 px-4 py-3">
                ...
                <p class="mt-2 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">{{ pickedGames }}/{{ totalGames }} picked</p>
              </div>
              ...
              <div v-if="isAuthenticated && currentBracket" class="rounded-2xl border border-border/70 bg-card/55 px-4 py-3">
                <p class="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Score</p>
      ...
      <section id="bracket-board" class="mx-auto max-w-[1500px] px-4 pb-24 sm:px-6 lg:px-8">
        ...
        <div v-if="allBracketMatchups.length" class="space-y-8">
          <article v-if="isAuthenticated && leaderboard.length" class="relative overflow-hidden rounded-[2rem] border border-border/70 bg-card/80 p-4 shadow-[0_20px_60px_-32px_rgba(15,23,42,0.18)] backdrop-blur sm:p-6">
            ...
            <div class="relative">
              <div class="flex items-end justify-between gap-4">
                <div>
                  <p class="text-xs uppercase tracking-[0.24em] text-muted-foreground">Leaderboard</p>
                  <h3 class="mt-2 text-xl font-semibold text-foreground sm:text-2xl">Bracket standings</h3>
                </div>
              ...
              <div class="mt-5 overflow-hidden rounded-[1.5rem] border border-border/70 bg-background/35">
                ...
                >
                  ...
                  <div class="text-right">
                    ...
                    <p class="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">pts</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            >
              <div class="flex items-center justify-between gap-3">
                <div>
                  <p class="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Region</p>
                  <p class="text-base font-semibold text-foreground">{{ region.name }}</p>
                </div>
                <div class="text-right">
                  <p class="text-sm font-semibold text-foreground">{{ regionCompletion(region).pct }}%</p>
                  <p class="text-[11px] text-muted-foreground">{{ regionCompletion(region).picked }}/{{ regionCompletion(region).total }}</p>
                </div>
              </div>
            </a>
          </div>
          ...
          <article class="relative overflow-hidden rounded-[2rem] border border-border/70 bg-card/80 p-4 shadow-[0_20px_60px_-32px_rgba(15,23,42,0.18)] backdrop-blur sm:p-6">
            ...
            <div class="relative">
              <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                  <p class="text-xs uppercase tracking-[0.24em] text-muted-foreground">Finals</p>
                  <h3 class="mt-2 text-xl font-semibold text-foreground sm:text-2xl">Final Four and Title Game</h3>
                </div>
                <p class="max-w-md text-sm leading-6 text-muted-foreground">Regional winners flow here automatically as you complete each board.</p>
              </div>

              <div class="mt-5 grid gap-5 xl:grid-cols-[1fr_0.92fr]">
                <section>
                  <p class="mb-3 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">{{ roundLabels.final_four }}</p>
                    ...
                    >
                      <div class="space-y-2">
                        <button
                          ...
                          :class="resultToneClass(matchup, matchup.participants[0])"
                          :disabled="isBracketLocked || !resolveSlotParticipant(matchup.participants[0])"
                          ...
                          <div class="flex items-center gap-2.5">
                            ...
                            <div class="min-w-0">
                              ...
                              <div class="mt-0.5 flex items-start gap-2">
                                ...
                                <div class="min-w-0">
                                  <p class="text-sm font-semibold leading-5 text-foreground">{{ participantButtonLabel(matchup.participants[0], 'Regional winner advances here') }}</p>
                                  <p v-if="slotWinProbability(matchup, matchup.participants[0]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[0])">{{ slotWinProbability(matchup, matchup.participants[0]) }}% ML</p>
                                  <p v-if="resultBadgeLabel(matchup, matchup.participants[0])" class="mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em]" :class="resultBadgeClass(matchup, matchup.participants[0])">{{ resultBadgeLabel(matchup, matchup.participants[0]) }}</p>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <button
                          type="button"
                          class="w-full rounded-xl border px-3 py-2 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-45"
                          :class="resultToneClass(matchup, matchup.participants[1])"
                          :disabled="isBracketLocked || !resolveSlotParticipant(matchup.participants[1])"
                          ...
                          <div class="flex items-center gap-2.5">
                            ...
                            <div class="min-w-0">
                              ...
                              <div class="mt-0.5 flex items-start gap-2">
                                ...
                                <div class="min-w-0">
                                  <p class="text-sm font-semibold leading-5 text-foreground">{{ participantButtonLabel(matchup.participants[1], 'Regional winner advances here') }}</p>
                                  <p v-if="slotWinProbability(matchup, matchup.participants[1]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[1])">{{ slotWinProbability(matchup, matchup.participants[1]) }}% ML</p>
                                  <p v-if="resultBadgeLabel(matchup, matchup.participants[1])" class="mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em]" :class="resultBadgeClass(matchup, matchup.participants[1])">{{ resultBadgeLabel(matchup, matchup.participants[1]) }}</p>
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <p class="mt-3 text-[11px] leading-5 text-muted-foreground">{{ formatVenue(matchup.game) }}</p>
                    </div>
                  </div>
                </section>
                  ...
                  <div class="rounded-[1.8rem] border border-border/70 bg-card/72 p-3.5 backdrop-blur sm:p-4">
                    <div class="space-y-2">
                      <button
                        ...
                        :class="resultToneClass(standaloneRounds.championship, standaloneRounds.championship.participants[0])"
                        :disabled="isBracketLocked || !resolveSlotParticipant(standaloneRounds.championship.participants[0])"
                        ...
                        <div class="flex items-center gap-2.5">
                          ...
                          <div class="min-w-0">
                            ...
                            <div class="mt-0.5 flex items-start gap-2">
                              ...
                              <div class="min-w-0">
                                <p class="text-base font-semibold leading-5 text-foreground">{{ participantButtonLabel(standaloneRounds.championship.participants[0], 'Semifinal winner advances here') }}</p>
                                <p v-if="slotWinProbability(standaloneRounds.championship, standaloneRounds.championship.participants[0]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(standaloneRounds.championship, standaloneRounds.championship.participants[0])">{{ slotWinProbability(standaloneRounds.championship, standaloneRounds.championship.participants[0]) }}% ML</p>
                                <p v-if="resultBadgeLabel(standaloneRounds.championship, standaloneRounds.championship.participants[0])" class="mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em]" :class="resultBadgeClass(standaloneRounds.championship, standaloneRounds.championship.participants[0])">{{ resultBadgeLabel(standaloneRounds.championship, standaloneRounds.championship.participants[0]) }}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <button
                        type="button"
                        class="w-full rounded-xl border px-3 py-2.5 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-45"
                        :class="resultToneClass(standaloneRounds.championship, standaloneRounds.championship.participants[1])"
                        :disabled="isBracketLocked || !resolveSlotParticipant(standaloneRounds.championship.participants[1])"
                        ...
                        <div class="flex items-center gap-2.5">
                          ...
                          <div class="min-w-0">
                            ...
                            <div class="mt-0.5 flex items-start gap-2">
                              ...
                              <div class="min-w-0">
                                <p class="text-base font-semibold leading-5 text-foreground">{{ participantButtonLabel(standaloneRounds.championship.participants[1], 'Semifinal winner advances here') }}</p>
                                <p v-if="slotWinProbability(standaloneRounds.championship, standaloneRounds.championship.participants[1]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(standaloneRounds.championship, standaloneRounds.championship.participants[1])">{{ slotWinProbability(standaloneRounds.championship, standaloneRounds.championship.participants[1]) }}% ML</p>
                                <p v-if="resultBadgeLabel(standaloneRounds.championship, standaloneRounds.championship.participants[1])" class="mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em]" :class="resultBadgeClass(standaloneRounds.championship, standaloneRounds.championship.participants[1])">{{ resultBadgeLabel(standaloneRounds.championship, standaloneRounds.championship.participants[1]) }}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <p class="mt-3 text-[11px] leading-5 text-muted-foreground">{{ formatVenue(standaloneRounds.championship.game) }}</p>
                  </div>
                </section>
              </div>
            </div>
          ...
          >
            ...
            <div class="relative space-y-5">
              <div class="flex flex-col gap-4 xl:grid xl:grid-cols-[220px_minmax(0,1fr)] xl:items-start">
                <div class="space-y-4">
                ...
                <div class="space-y-4">
                  <div class="grid gap-2 sm:grid-cols-2 lg:hidden">
                    ...
                    >
                      <div class="mb-3 flex items-center justify-between gap-3 border-b border-border/60 pb-2">
                        ...
                        <div class="text-right">
                          <p class="text-[11px] text-muted-foreground">{{ round.matchups.length }}</p>
                          <p
                            v-if="isAuthenticated && currentBracket"
                      ...
                      <div class="space-y-3">
                        ...
                        >
                          <div
                            v-if="matchup.participants.some((slot) => slot.sourceMatchupId)"
                            class="mb-1.5 inline-flex rounded-full border border-dashed border-border/70 bg-card/60 px-2.5 py-1 text-[9px] uppercase tracking-[0.16em] text-foreground/75"
                          >
                            {{ isRoundOf64PlayInCard(matchup) ? 'First Four' : 'Play-in winner feeds this slot' }}
                          </div>
                          <div
                            v-if="compactMatchupSummary(matchup)"
                            class="mb-2 text-[10px] leading-4 text-foreground/70"
                            :class="isRoundOf64PlayInCard(matchup) ? 'line-clamp-2' : ''"
                          >
                            {{ compactMatchupSummary(matchup) }}
                          </div>
                          ...
                          <div class="space-y-2">
                            <button
                              ...
                              :class="selectedSlotResultStatus(matchup, matchup.participants[0]) ? resultToneClass(matchup, matchup.participants[0]) : slotIsPlaceholder(matchup.participants[0])
                              ...
                              <div class="flex items-center gap-2.5">
                                ...
                                <div class="min-w-0">
                                  ...
                                  <div class="mt-0.5 flex items-start gap-2">
                                    <span
                                      v-if="participantSeed(matchup.participants[0])"
                                      class="inline-flex min-w-6 justify-center rounded-full border border-border/70 bg-card/80 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-muted-foreground"
                                    >
                                      {{ participantSeed(matchup.participants[0]) }}
                                    </span>
                                    <div class="min-w-0">
                                      <p class="text-sm font-semibold leading-5" :class="slotIsPlaceholder(matchup.participants[0]) ? 'text-muted-foreground' : 'text-foreground'">{{ participantButtonLabel(matchup.participants[0], 'Winner advances here') }}</p>
                                      <p v-if="slotWinProbability(matchup, matchup.participants[0]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[0])">{{ slotWinProbability(matchup, matchup.participants[0]) }}% ML</p>
                                    </div>
                                  </div>
                                </div>
                            ...
                            <button
                              type="button"
                              class="w-full rounded-xl border px-2.5 py-2 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-45"
                              :class="selectedSlotResultStatus(matchup, matchup.participants[1]) ? resultToneClass(matchup, matchup.participants[1]) : slotIsPlaceholder(matchup.participants[1])
                                  ? 'border-border/60 bg-card/55 text-muted-foreground hover:border-border/80 hover:bg-card/70'
                                  : 'border-border/70 bg-card/75 hover:border-border hover:bg-accent/60'"
                              :disabled="isBracketLocked || !resolveSlotParticipant(matchup.participants[1])"
                              @click="selectWinner(matchup, matchup.participants[1])"
                              ...
                              <div class="flex items-center gap-2.5">
                                ...
                                <div class="min-w-0">
                                  ...
                                  <div class="mt-0.5 flex items-start gap-2">
                                    ...
                                    <div class="min-w-0">
                                      <p class="text-sm font-semibold leading-5" :class="slotIsPlaceholder(matchup.participants[1]) ? 'text-muted-foreground' : 'text-foreground'">{{ participantButtonLabel(matchup.participants[1], 'Winner advances here') }}</p>
                                      <p v-if="slotWinProbability(matchup, matchup.participants[1]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[1])">{{ slotWinProbability(matchup, matchup.participants[1]) }}% ML</p>
                                    </div>
                                  </div>
                                </div>
                  ...
                  <div class="hidden overflow-x-auto pb-2 [scrollbar-width:thin] lg:block">
                    <div class="grid min-w-[1060px] grid-cols-4 gap-4 xl:gap-5">
                      <div
                        v-for="round in region.rounds"
                        :key="`${region.id}-${round.key}`"
                        class="flex flex-col gap-4"
                        :class="roundColumnMinHeightClass(round.key)"
                      >
                        <div class="rounded-2xl border px-3 py-2" :class="roundSectionClass(region, round.key)">
                        ...
                        <div class="grid flex-1 gap-2.5 content-start overflow-visible xl:gap-3" :class="roundColumnGridClass(round.key)">
                          <div
                            v-for="(matchup, matchupIndex) in round.matchups"
                            :key="matchup.id"
                            class="relative self-start rounded-2xl border border-border/70 bg-card/70 p-2 backdrop-blur xl:p-2.5"
                            :style="roundTrackStyle(round.key, matchupIndex)"
                          >
                            <div
                              v-if="matchup.participants.some((slot) => slot.sourceMatchupId)"
                              class="mb-1 rounded-xl border border-dashed border-border/70 bg-card/60 px-2 py-1 text-[9px] uppercase tracking-[0.16em] text-foreground/75"
                            >
                              {{ isRoundOf64PlayInCard(matchup) ? 'First Four' : 'Play-in winner feeds this slot' }}
                            </div>
                            <div
                              ...
                              :class="isRoundOf64PlayInCard(matchup) ? 'line-clamp-1' : ''"
                            >
                              {{ compactMatchupSummary(matchup) }}
                            </div>

                            <div :class="isRoundOf64PlayInCard(matchup) ? 'space-y-1' : 'space-y-1.25'">
                              <button
                                type="button"
                                class="w-full rounded-xl border text-left transition-colors disabled:cursor-not-allowed disabled:opacity-45"
                                :class="[
                                  isRoundOf64PlayInCard(matchup) ? 'px-2 py-1' : 'px-2.5 py-1',
                                  resultToneClass(matchup, matchup.participants[0]),
                                  slotIsPlaceholder(matchup.participants[0]) ? 'border-border/60 bg-card/55 text-muted-foreground hover:border-border/80 hover:bg-card/70' : '',
                                ]"
                                :disabled="isBracketLocked || !resolveSlotParticipant(matchup.participants[0])"
                                @click="selectWinner(matchup, matchup.participants[0])"
                              >
                                <div class="flex items-center gap-2">
                                  ...
                                  <div class="min-w-0">
                                    ...
                                    <div class="mt-0.5 flex items-start gap-1.5">
                                      <span
                                        v-if="participantSeed(matchup.participants[0])"
                                        class="inline-flex min-w-5 justify-center rounded-full border border-border/70 bg-card/80 px-1 py-0.5 text-[9px] font-semibold leading-none text-muted-foreground"
                                      >
                                        {{ participantSeed(matchup.participants[0]) }}
                                      </span>
                                      <div class="min-w-0">
                                        <p
                                          class="font-semibold"
                                          :class="[
                                            isRoundOf64PlayInCard(matchup) ? 'text-[13px] leading-4' : 'text-sm leading-4',
                                            slotIsPlaceholder(matchup.participants[0]) ? 'text-muted-foreground' : 'text-foreground',
                                          ]"
                                        >
                                          {{ participantButtonLabel(matchup.participants[0], 'Winner advances here') }}
                                        </p>
                                        <p v-if="slotWinProbability(matchup, matchup.participants[0]) !== null" class="text-[10px]" :class="slotWinProbabilityClass(matchup, matchup.participants[0])">{{ slotWinProbability(matchup, matchup.participants[0]) }}% ML</p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              ...
                              <button
                                ...
                                :class="[
                                  isRoundOf64PlayInCard(matchup) ? 'px-2 py-1' : 'px-2.5 py-1',
                                  resultToneClass(matchup, matchup.participants[1]),
                                  slotIsPlaceholder(matchup.participants[1]) ? 'border-border/60 bg-card/55 text-muted-foreground hover:border-border/80 hover:bg-card/70' : '',
                                ]"
                                :disabled="isBracketLocked || !resolveSlotParticipant(matchup.participants[1])"
                                @click="selectWinner(matchup, matchup.participants[1])"
                              >
                                    ...
                                    <div class="mt-0.5 flex items-start gap-1.5">
                                      ...
                                      <div class="min-w-0">
                                        <p
                                          ...
                                          :class="[
                                            isRoundOf64PlayInCard(matchup) ? 'text-[13px] leading-4' : 'text-sm leading-4',
                                            slotIsPlaceholder(matchup.participants[1]) ? 'text-muted-foreground' : 'text-foreground',
                                          ]"
                                        >
                                          {{ participantButtonLabel(matchup.participants[1], 'Winner advances here') }}
                                        </p>
                                        <p v-if="slotWinProbability(matchup, matchup.participants[1]) !== null" class="text-[10px]" :class="slotWinProbabilityClass(matchup, matchup.participants[1])">{{ slotWinProbability(matchup, matchup.participants[1]) }}% ML</p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
            ...
            <div class="relative">
              ...
              <div class="mt-5 grid gap-4 lg:grid-cols-2">
                <div
                  ...
                  class="rounded-2xl border border-border/70 bg-card/70 p-3 backdrop-blur"
                >
                  <div class="mb-3 inline-flex rounded-full border border-border/70 bg-card/60 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-foreground/75">
                    Winner to {{ firstFourDestinationLabel(matchup) }}
                  </div>
                      ...
                      <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                          ...
                          <div class="mt-0.5 flex items-start gap-2">
                            <span
                              v-if="participantSeed(matchup.participants[0])"
                              class="inline-flex min-w-7 justify-center rounded-full border border-border/70 bg-card/80 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-muted-foreground"
                            >
                              {{ participantSeed(matchup.participants[0]) }}
                            </span>
                            <div class="min-w-0">
                              <p class="text-sm font-semibold leading-5 text-foreground">{{ participantButtonLabel(matchup.participants[0], 'TBD') }}</p>
                              <p v-if="slotWinProbability(matchup, matchup.participants[0]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[0])">{{ slotWinProbability(matchup, matchup.participants[0]) }}% ML</p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                          ...
                          <div class="mt-0.5 flex items-start gap-2">
                            ...
                            <div class="min-w-0">
                              <p class="text-sm font-semibold leading-5 text-foreground">{{ participantButtonLabel(matchup.participants[1], 'TBD') }}</p>
                              <p v-if="slotWinProbability(matchup, matchup.participants[1]) !== null" class="text-[11px]" :class="slotWinProbabilityClass(matchup, matchup.participants[1])">{{ slotWinProbability(matchup, matchup.participants[1]) }}% ML</p>
                            </div>
                          </div>
                        </div>
        ...
        <div v-else class="rounded-[2rem] border border-dashed border-border/70 bg-card/55 p-8 text-sm leading-6 text-muted-foreground">
          No NCAA tournament games are currently available for the bracket board. Once the synced CBB games have tournament metadata, this view will populate from the database automatically.
        </div>
      </section>
    </main>

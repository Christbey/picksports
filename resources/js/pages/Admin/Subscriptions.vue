...
function syncUser(userId: number) {
    ...
    router.post(
        `/admin/subscriptions/${userId}/sync`,
        {},
        {
            onFinish: () => {
...
function syncAll() {
    if (
        !confirm(
            'Are you sure you want to sync all subscriptions with Stripe? This may take a while.',
        )
    ) {
        return;
    }
...
function assignTier(userId: number, tierSlug: string) {
    ...
    if (
        !confirm(
            `Are you sure you want to assign ${tierSlug} tier to this user?`,
        )
    ) {
        return;
    }
    ...
    router.post(
        `/admin/subscriptions/${userId}/assign-tier`,
        {
            tier_slug: tierSlug,
        },
        {
            ...
            >
                ...
                <div class="flex gap-4">
                    <input
                        ...
                        class="flex-1 rounded-lg border border-sidebar-border bg-white px-4 py-2 focus:ring-2 focus:ring-primary focus:outline-none dark:bg-sidebar"
                    />
                </div>
                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="p-4">
                                    <span
                                        class="inline-block rounded-full bg-sidebar-accent px-3 py-1 text-sm font-medium capitalize"
                                    >
                                        {{ user.tier }}
                                    </span>
                                </td>
                                <td class="p-4">
                                    <select
                                        ...
                                        "
                                        :disabled="
                                            assigningTierUserId === user.id
                                        "
                                        class="rounded-lg border border-sidebar-border bg-white px-3 py-1 text-sm focus:ring-2 focus:ring-primary focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:bg-sidebar"
                                    >
                ...
                >
                    ...
                    <div class="flex gap-2">
                        <button
                            ...
                            @click="
                                router.get(
                                    `/admin/subscriptions?page=${page}&search=${search}`,
                                )
                            "
                            :class="[

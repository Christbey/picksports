...
{
    protected $signature = 'espn:sync-wcbb-games-scoreboard
                            {date? : The date in YYYYMMDD format (defaults to today)}
                            {--from-date= : Start date in YYYY-MM-DD format}
                            {--to-date= : End date in YYYY-MM-DD format}
                            {--season= : Sync entire season (e.g., 2026 syncs Nov 2025 - Apr 2026)}';

    protected $description = 'Sync WCBB games from ESPN scoreboard API';
    ...
    public function handle(): int
    {
        ...
        $this->info("Dispatching WCBB games scoreboard sync job for date {$date}...");

        FetchGamesFromScoreboard::dispatch($date);

        $this->info('WCBB games scoreboard sync job dispatched successfully.');
    ...
    {
        // WCBB season runs from November (previous year) to April (current year)
        $startDate = Carbon::create($season - 1, 11, 1)->startOfMonth();
        $endDate = Carbon::create($season, 4, 30)->endOfMonth();

        $this->info("Syncing full {$season} WCBB season ({$startDate->format('Y-m-d')} to {$endDate->format('Y-m-d')})...");

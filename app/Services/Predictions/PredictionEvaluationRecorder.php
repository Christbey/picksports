...
class PredictionEvaluationRecorder
{
    ...
    {
        $actualHomeWin = $actualSpread > 0 ? 1.0 : 0.0;
        ...
        $marketSpreadError = $marketSpread === null ? null : abs($actualSpread - $marketSpread);
    ...
    private function extractMarketSpread(Model $prediction): ?float
    {
        foreach (['vegas_spread', 'market_spread'] as $field) {
            if (is_numeric($prediction->{$field} ?? null)) {
    ...
    {
        return $probability === null ? null : abs($actualHomeWin - $probability);
    }

    private function brierScore(float $actualHomeWin, ?float $probability): ?float
    {
        return $probability === null ? null : ($actualHomeWin - $probability) ** 2;
    }

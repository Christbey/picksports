...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Team Metrics', href: '/wcbb-team-metrics' },
    { title: props.team.name, href: `/wcbb/teams/${props.team.id}` }
]
...
onMounted(async () => {
    try {
        ...
        const [metricsRes, gamesRes] = await Promise.all([
            fetch(`/api/v1/wcbb/teams/${props.team.id}/metrics`),
            fetch(`/api/v1/wcbb/teams/${props.team.id}/games`)
        ])
                    ...
                    <CardContent>
                        <div class="space-y-2">
                            <Link
                                ...
                                :key="game.id"
                                :href="WCBBGameController(game.id)"

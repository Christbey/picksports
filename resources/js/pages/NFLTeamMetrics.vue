...
const sortBy = ref<'net_rating' | 'offensive_rating' | 'defensive_rating'>('net_rating');
...
const fetchMetrics = async () => {
    try {
        ...
        const response = await fetch('/api/v1/nfl/team-metrics');
...
const getTurnoverClass = (value: number | null): string => {
    if (value === null) return '';
    if (value > 5) return 'text-green-600 dark:text-green-400 font-semibold';
    if (value > 0) return 'text-green-600 dark:text-green-400';
    if (value < -5) return 'text-red-600 dark:text-red-400 font-semibold';
...
<template>
    <Head title="NFL Team Metrics" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">NFL Team Metrics</h1>
                    <p class="text-sm text-muted-foreground">
                        Advanced metrics for NFL teams
                    </p>
                </div>
            </div>

            <SubscriptionBanner variant="subtle" storage-key="nfl-metrics-banner-dismissed" />
                ...
                <CardContent class="pt-6">
                    <div class="flex flex-wrap items-end gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            >
                                Net {{ sortBy === 'net_rating' ? (sortDesc ? '↓' : '↑') : '' }}
                            </Button>
                            <Button
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b text-left">
                                    ...
                                    <th class="p-2 font-medium text-right">PPG</th>
                                    <th class="p-2 font-medium text-right">PA/G</th>
                                    <th class="p-2 font-medium text-right">Net</th>
                                    <th class="p-2 font-medium text-right">YPG</th>
                                    <th class="p-2 font-medium text-right">YA/G</th>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td class="p-2 font-medium">
                                        <Link
                                            :href="NFLTeamController(metric.team.id)"
                                    ...
                                    <td class="p-2 text-right text-muted-foreground">
                                        {{ formatNumber(metric.yards_per_game) }}
                                    </td>
                                    <td class="p-2 text-right text-muted-foreground">
                                        {{ formatNumber(metric.yards_allowed_per_game) }}
                                    </td>
                                    <td class="p-2 text-right" :class="getTurnoverClass(metric.turnover_differential)">
                                        {{ formatNumber(metric.turnover_differential, 0) }}
                                    </td>
                                    <td class="p-2 text-right text-muted-foreground">

...
import WNBAGameController from '@/actions/App/Http/Controllers/WNBA/GameController'
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Team Metrics', href: '/wnba-team-metrics' },
    { title: props.team.name, href: `/wnba/teams/${props.team.id}` }
]
...
onMounted(async () => {
    try {
        ...
        const [metricsRes, gamesRes] = await Promise.all([
            fetch(`/api/v1/wnba/teams/${props.team.id}/metrics`),
            fetch(`/api/v1/wnba/teams/${props.team.id}/games`)
        ])
                    ...
                    <CardContent>
                        <div class="space-y-2">
                            <Link
                                ...
                                :key="game.id"
                                :href="WNBAGameController(game.id)"

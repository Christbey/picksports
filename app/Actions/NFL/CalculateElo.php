...
class CalculateElo
{
    public function execute(Game $game, bool $skipIfExists = true): array
    {
        ...
        $homeElo = $homeTeam->elo_rating ?? config('nfl.elo.default_rating');
        $awayElo = $awayTeam->elo_rating ?? config('nfl.elo.default_rating');

        // Adjust for home field advantage
        $adjustedHomeElo = $homeElo + config('nfl.elo.home_field_advantage');
    ...
    {
        // Check if the game is a playoff game based on season_type

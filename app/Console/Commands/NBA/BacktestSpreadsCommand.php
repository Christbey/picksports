...
{
    protected $signature = 'nba:backtest-spreads
    ...
    protected $description = 'Backtest NBA spread predictions against market spreads and final margins';
    ...
    public function handle(): int
    {
        ...
        if ($rows->isEmpty()) {
            $this->warn('No final NBA predictions with spread market data found for the selected scope.');
        ...
        $this->info('NBA Spreads Backtest');
    ...
    private function loadRows(): Collection
    {
        ...
        return $query->get()
            ->map(function (Prediction $prediction) use ($generator): ?array {
                ...
                if (! $game || ! is_numeric($prediction->vegas_spread)) {
                    return null;
                }
    ...
    {
        $threshold = (float) config('nba.betting.edge_thresholds.spread');
    ...
    {
        $buckets = [
            '1.5-2.9' => fn (float $edge) => $edge >= 1.5 && $edge < 3.0,

...
{
    protected $signature = 'espn:sync-cbb-games-scoreboard
    ...
    protected $description = 'Sync CBB games from ESPN scoreboard API';
    ...
    public function handle(): int
    {
        ...
        $this->info("Dispatching CBB games scoreboard sync job for date {$date}...");

        FetchGamesFromScoreboard::dispatch($date);

        $this->info('CBB games scoreboard sync job dispatched successfully.');
    ...
    {
        // CBB season runs from November (previous year) to April (current year)
        $startDate = Carbon::create($season - 1, 11, 1)->startOfMonth();
        $endDate = Carbon::create($season, 4, 30)->endOfMonth();

        $this->info("Syncing full {$season} CBB season ({$startDate->format('Y-m-d')} to {$endDate->format('Y-m-d')})...");

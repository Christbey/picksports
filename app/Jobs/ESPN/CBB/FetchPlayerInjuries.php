...
use App\Actions\ESPN\CBB\SyncPlayerInjuries;
...
class FetchPlayerInjuries implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->teamEspnId) {
            ...
            Log::info("CBB: Synced {$count} player injuries for team {$this->teamEspnId} from ESPN");
        } else {
            $count = $action->syncAllTeams();
            Log::info("CBB: Synced {$count} total player injuries from ESPN");
        }
    }
}

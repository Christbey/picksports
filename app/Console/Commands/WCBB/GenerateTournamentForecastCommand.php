...
{
    protected $signature = 'wcbb:generate-tournament-forecast
        {--season= : Season to forecast (defaults to wcbb.season.default)}
        {--simulations= : Number of Monte Carlo simulations to run}';

    protected $description = 'Generate WCBB March Madness field and champion probability forecasts';
    ...
    public function handle(GenerateTournamentForecast $generateTournamentForecast): int
    {
        ...
        $this->info('Generating WCBB tournament forecast...');
        ...
        if ($forecasts->isEmpty()) {
            $this->warn('No eligible WCBB team metrics found. Run wcbb:calculate-team-metrics first.');

            return self::SUCCESS;
        }

        $resolvedSeason = (int) ($season ?? config('wcbb.season.default'));

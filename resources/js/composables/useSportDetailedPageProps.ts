...
export interface UseSportDetailedPagePropsOptions {
    ...
    formatNumber?: MaybeRefOrGetter<
        (value: number | string | null | undefined, decimals?: number) => string
    >;
...
export function useSportDetailedPageProps(
    options: UseSportDetailedPagePropsOptions,
) {
    ...
    return computed(() => {
        ...
        return {
            ...
            awayRecentForm: options.awayRecentForm
                ? toValue(options.awayRecentForm)
                : undefined,
            homeRecentForm: options.homeRecentForm
                ? toValue(options.homeRecentForm)
                : undefined,
            venueLabel: options.venueLabel
                ? toValue(options.venueLabel)
                : undefined,
            broadcastNetworks: options.broadcastNetworks
                ? toValue(options.broadcastNetworks)
                : [],
            extraInfoItems: options.extraInfoItems
                ? toValue(options.extraInfoItems)
                : [],
            showScoreStatuses: options.showScoreStatuses
                ? toValue(options.showScoreStatuses)
                : ['STATUS_FINAL'],
            badgePulseStatuses: options.badgePulseStatuses
                ? toValue(options.badgePulseStatuses)
                : [],
            linkTeams: options.linkTeams ? toValue(options.linkTeams) : true,
            useTeamColorGlow: options.useTeamColorGlow
                ? toValue(options.useTeamColorGlow)
                : false,
            ...
            linescoreTitle: options.linescoreTitle
                ...
                : 'Linescore',
            awayLinescores: options.awayLinescores
                ? toValue(options.awayLinescores)
                : [],
            homeLinescores: options.homeLinescores
                ? toValue(options.homeLinescores)
                : [],
            ...
            projectedLabel: options.projectedLabel
                ? toValue(options.projectedLabel)
                : config.projectedLabel || 'Projected points',
            awayBarClass: options.awayBarClass
                ? toValue(options.awayBarClass)
                : config.awayBarClass || 'bg-blue-500 dark:bg-blue-600',
            homeBarClass: options.homeBarClass
                ? toValue(options.homeBarClass)
                : config.homeBarClass || 'bg-blue-800 dark:bg-blue-400',
            showTrends: options.showTrends
                ? toValue(options.showTrends)
                : false,
            trendsTitle: options.trendsTitle
                ? toValue(options.trendsTitle)
                : config.trendsTitle || 'Team Trends',
            trendsSubtitle: options.trendsSubtitle
                ? toValue(options.trendsSubtitle)
                : undefined,
            trendsLoading: options.trendsLoading
                ? toValue(options.trendsLoading)
                : false,
            topMatchupEdges: options.topMatchupEdges
                ? toValue(options.topMatchupEdges)
                : [],
            allTrendCategories: options.allTrendCategories
                ? toValue(options.allTrendCategories)
                : [],
            ...
            trendsEmptyText: options.trendsEmptyText
                ? toValue(options.trendsEmptyText)

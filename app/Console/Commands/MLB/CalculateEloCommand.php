...
class CalculateEloCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($this->option('reset')) {
            $this->info('Resetting all team and pitcher Elo ratings to 1500...');
            Team::query()->update(['elo_rating' => 1500]);
            Player::query()->update(['elo_rating' => 1500]);
            EloRating::query()->truncate();
            PitcherEloRating::query()->truncate();
        ...
        foreach ($games as $game) {
            ...
            if ($result['skipped']) {
                $totalSkipped++;
            } elseif ($result['home_team_change'] != 0 || $result['away_team_change'] != 0) {
                $totalCalculated++;
            }
        ...
        // Show top pitchers by Elo
        $this->newLine();
        $this->info('Top 10 Pitchers by Elo Rating:');

        $topPitchers = Player::query()
        ...
        $this->table(
            ['Rank', 'Pitcher', 'Team', 'Elo Rating'],
            $topPitchers->map(fn ($pitcher, $index) => [
                $index + 1,
                $pitcher->full_name,
                $pitcher->team?->abbreviation ?? 'N/A',
                $pitcher->elo_rating,
            ])
        );

...
const edgeValue = (row: TournamentForecast) =>
...
const topByMake = computed(() =>
    [...visibleForecasts.value]
        .sort((a, b) => {
            ...
            return (
                b.tournament_make_probability - a.tournament_make_probability
            );
        })
        .slice(0, 24),
);
...
const bubbleRisk = computed(() =>
    [...visibleForecasts.value]
        .filter(
            (f) =>
                f.tournament_make_probability > 0.05 &&
                f.tournament_make_probability < 0.95,
        )
...
const conferenceBreakdown = computed(() => {
    ...
    for (const row of visibleForecasts.value) {
        const conference =
            (row.team?.conference ?? 'Unknown').trim() || 'Unknown';
        ...
        if (
            row.auto_bid_probability > current.predictedWinnerAutoBidProbability
        ) {
            current.predictedWinner = formatTeam(row.team, row.team_id);
            current.predictedWinnerAutoBidProbability =
                row.auto_bid_probability;
        }
    ...
    return Array.from(byConference.values())
        .map((row) => ({
            ...
            avgMakeProbability:
                row.teamsTracked > 0 ? row.expectedBids / row.teamsTracked : 0,
            predictedWinnerAutoBidProbability: Number(
                row.predictedWinnerAutoBidProbability.toFixed(4),
            ),
        }))
        .sort((a, b) => {
            if (b.projectedIn !== a.projectedIn)
                return b.projectedIn - a.projectedIn;
            if (b.expectedBids !== a.expectedBids)
                return b.expectedBids - a.expectedBids;
...
const projectedInCount = computed(
    () =>
        visibleForecasts.value.filter(
            (f) => f.tournament_make_probability >= 0.5,
        ).length,
);
const bubbleCount = computed(
    () =>
        visibleForecasts.value.filter(
            (f) =>
                f.tournament_make_probability >= 0.35 &&
                f.tournament_make_probability < 0.75,
        ).length,
);
const avgBidThiefRisk = computed(() => {
    ...
    return (
        visibleForecasts.value.reduce(
            (sum, row) => sum + row.bid_thief_probability,
            0,
...
<template>
    <Head title="WCBB Tournament Forecast" />

    <PredictionsPageShell
        title="WCBB Tournament Forecast"
        sport-title="WCBB"
        sport-href="/wcbb/predictions"
        ...
        <div class="space-y-4">
            <Card>
                <CardContent class="pt-6">
                    <div class="flex flex-wrap items-end gap-4">
                        ...
                        <div class="min-w-[220px]">
                            <label for="sort-mode" class="text-sm font-medium"
                                >Sort</label
                            >
                            <select
                                id="sort-mode"
                        ...
                        <label
                            class="mt-6 inline-flex items-center gap-2 text-sm font-medium text-muted-foreground"
                        >
                            <input
                                v-model="onlyPlusEv"
                                type="checkbox"
            ...
            <template v-else>
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <Card>
                        <CardContent class="pt-5">
                            ...
                            <p class="mt-1 text-xs text-muted-foreground">
                                Teams with make probability 50%+
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                    ...
                    <Card>
                        <CardContent class="pt-5">
                            ...
                            <p class="mt-2 text-3xl font-bold">
                                {{ formatPct(avgBidThiefRisk, 2) }}
                            </p>
                            <p class="mt-1 text-xs text-muted-foreground">
                                Across all forecasted teams
                            </p>
                        </CardContent>
                    </Card>
                </div>
                                    ...
                                    >
                                        ...
                                        <td class="p-2">
                                            <div class="flex flex-wrap gap-1.5">
                                                <span
                                                    class="rounded bg-blue-100 px-1.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-200"
                                                    >AQ
                                                    {{
                                                        formatPct(
                                                            row.auto_bid_probability,
                                                ...
                                                >
                                                <span
                                                    class="rounded bg-indigo-100 px-1.5 py-0.5 text-xs font-medium text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-200"
                                                    >AL
                                                    {{
                                                        formatPct(
                                                            row.at_large_probability,
                                                        )
                                                    }}</span
                                                >
                                                <span
                                                    class="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-200"
                                                    >F4
                                                    {{
                                                        formatPct(
                                                            row.first_four_probability,
                                                        )
                                                    }}</span
                                                >
                                            </div>
                                            ...
                                            >
                                                ...
                                                >
                                                    <div
                                                        ...
                                                        "
                                                        :style="{
                                                            width: `${toPct(row.tournament_make_probability)}%`,
                                                        }"
                                                    />
                                                </div>
                                                <span
                                        ...
                                        <td class="p-2">
                                            <div class="flex flex-col">
                                                ...
                                                <span
                                                    ...
                                                    {{
                                                        formatPct(
                                                            row.predictedWinnerAutoBidProbability,
                                                            1,
                                                        )
                                                    }}</span
                                                >
                                ...
                                <div>
                                    ...
                                    >
                                        <div
                                            ...
                                            :style="{
                                                width: `${toPct(row.final_four_probability)}%`,
                                            }"
                                        />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    >
                                        <div
                                            ...
                                            :style="{
                                                width: `${toPct(row.title_game_probability)}%`,
                                            }"
                                        />
                                    </div>
                                </div>
                ...
                >
                    ...
                    <CardContent>
                        <div
                            class="overflow-x-auto rounded-md border border-amber-200/50 dark:border-amber-900/40"
                        >
                            <table class="w-full text-sm">
                                <thead>
                                    <tr
                                        class="border-b bg-amber-50/60 text-left dark:bg-amber-950/20"
                                    >
                                        <th class="p-2 font-medium">Team</th>
                                        <th class="p-2 font-medium">Status</th>
                                        <th class="p-2 text-right font-medium">
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td class="p-2 text-right">
                                            {{
                                                formatPct(
                                                    row.first_four_auto_probability,
                                                    2,
                                                )
                                            }}
                                        </td>
                                        <td class="p-2 text-right">
                                            {{
                                                formatPct(
                                                    row.first_four_at_large_probability,
                                                    2,
                                                )
                                            }}
                                        </td>

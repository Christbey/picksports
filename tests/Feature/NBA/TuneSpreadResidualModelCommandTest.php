...
it('tunes the spread residual challenger model across ridge values', function () {
    $inputDir = storage_path('app/ml/tune-model-splits');
    $outputPath = storage_path('app/ml/models/test_nba_spread_residual_model_tuned.json');
    ...
    Artisan::call('nba:tune-spread-residual-model', [
        '--input-dir' => $inputDir,
        '--output' => $outputPath,
        '--ridge-values' => '0,0.1,1,10',
    ]);
    ...
    expect($artifact)->toBeArray()
        ...
        ->and($artifact)->toHaveKey('best_ridge_lambda')
        ->and($artifact['sweep_results'])->toHaveCount(4)

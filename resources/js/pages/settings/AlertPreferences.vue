...
const selectedNotificationLabels = computed(() =>
    selectedNotificationTypes.value.map(
        (type) => notificationLabelMap.value[type] ?? type.toUpperCase(),
    ),
);
const autoDigestMode = computed(() =>
    selectedNotificationTypes.value.includes('push')
        ? 'realtime'
        : 'daily_summary',
);
const digestModeLabel = computed(() =>
    autoDigestMode.value === 'daily_summary' ? 'Daily Summary' : 'Real-time',
);
const needsPhoneNumber = computed(
    () =>
        selectedNotificationTypes.value.includes('whatsapp') ||
        selectedNotificationTypes.value.includes('sms'),
);
...
const webPushHasSubscription = ref<boolean>(
    props.webPush?.hasSubscription ?? false,
);
...
async function enableWebPush() {
    ...
    try {
        ...
        const response = await fetch(webPushRoutes.subscriptions.store.url(), {
            method: 'POST',
            headers: {
    ...
    try {
        if ('serviceWorker' in navigator) {
            ...
            if (subscription) {
                ...
                await fetch(webPushRoutes.subscriptions.destroy.url(), {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': getCsrfToken(),
...
async function sendTestPush() {
    ...
    try {
        const response = await fetch(webPushRoutes.test.url(), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': getCsrfToken(),
                Accept: 'application/json',
        ...
        if (!response.ok || !data.ok) {
            throw new Error(
                data.message ?? 'Failed to send test push notification.',
            );
        }
            ...
            <RenderErrorBoundary title="Alert Preferences Render Error">
                <div class="flex flex-col space-y-6">
                    ...
                    >
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="grid gap-1.5 leading-none">
                                    ...
                                    <p class="text-sm text-muted-foreground">
                                        Receive notifications when we identify
                                        high-value betting opportunities based
                                        on your preferences
                                    </p>
                                </div>
                            </div>
                        ...
                        <div
                            class="rounded-xl border border-sidebar-border bg-sidebar-accent/40 p-4"
                        >
                        ...
                        >
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                <div
                                    ...
                                    :class="[
                                        ...
                                        isNotificationTypeSelected(type.value)
                                ...
                                >
                                    ...
                                    <span
                                        v-if="type.disabled"
                                        class="ml-auto rounded bg-sidebar-accent px-2 py-1 text-xs text-muted-foreground"
                                    >
                                        Soon
                                    </span>
                                </div>
                            ...
                            <InputError
                                class="mt-2"
                                :message="errors.notification_types"
                            />
                        </div>
                        ...
                        >
                            <div>
                                <Label class="text-base"
                                    >Web Push Setup (iOS Home Screen)</Label
                                >
                                <p class="text-sm text-muted-foreground">
                                    Enable browser push for this device. On
                                    iPhone/iPad, install this app to the Home
                                    Screen first.
                                </p>
                            </div>
                            ...
                            >
                                ...
                                >
                                    iOS push only works for the installed Home
                                    Screen app, not Safari tabs.
                                </p>
                                <p>
                                    Device subscription:
                                    <span class="font-medium text-foreground">
                                        {{
                                            webPushHasSubscription
                                                ? 'Active'
                                                : 'Not active'
                                        }}
                                    </span>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <Button
                                    ...
                                    variant="outline"
                                    :disabled="
                                        webPushBusy || !webPushHasSubscription
                                    "
                                    @click="sendTestPush"
                                >
                                    Send Test Push
                            ...
                            <p
                                v-if="!props.webPush.configured"
                                class="text-sm text-amber-600 dark:text-amber-400"
                            >
                                Push notifications are not configured on the
                                server yet. Add VAPID keys in environment
                                settings.
                            </p>
                            <p
                                v-if="webPushMessage"
                        ...
                        <input
                            ...
                            name="minimum_edge"
                            :value="
                                String(currentPreference.minimum_edge ?? '0')
                            "
                        />
                        ...
                        <input
                            ...
                            :value="currentPreference.time_window_start"
                        />
                        <input
                            type="hidden"
                            name="time_window_end"
                            :value="currentPreference.time_window_end"
                        />
                        <input
                            type="hidden"
                            name="digest_mode"
                            :value="autoDigestMode"
                        />
                        <input
                            type="hidden"
                            name="digest_time"
                            :value="currentPreference.digest_time || '10:00'"
                        />

                        <!-- Phone Number -->
                        <div
                            class="space-y-3"
                            :class="!needsPhoneNumber ? 'opacity-60' : ''"
                        >
                            <div>
                                <Label for="phone_number" class="text-base"
                                    >Phone Number (Optional)</Label
                                >
                                <p class="text-sm text-muted-foreground">
                                    Required for WhatsApp alerts. Use full
                            ...
                            <Input
                                ...
                                type="tel"
                                :default-value="
                                    currentPreference.phone_number || ''
                                "
                                class="max-w-xs"

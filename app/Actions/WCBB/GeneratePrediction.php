...
class GeneratePrediction extends AbstractPredictionGenerator
{
    ...
    private function getRecentForm(Team $team, int $season): array
    {
        ...
        $stats = TeamStat::query()
            ...
            ->join('wcbb_games', 'wcbb_team_stats.game_id', '=', 'wcbb_games.id')
            ->orderByDesc('wcbb_games.game_date')
            ->select('wcbb_team_stats.*')
            ->get();
    ...
    {
        $defaultEff = config('wcbb.prediction.default_efficiency');
    ...
    private function isHomeTeamOutcome(string $outcomeName, Model $game): bool
    {
        ...
        $school = strtolower($homeTeam->school ?? '');
        $mascot = strtolower($homeTeam->mascot ?? '');

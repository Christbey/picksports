...
{
    protected $signature = 'cbb:grade-player-props
                            {--season= : Grade player props for a specific season (defaults to current year)}';

    protected $description = 'Grade CBB player props against actual player statistics';
    ...
    public function handle(): int
    {
        ...
        $this->info("Grading CBB player props for season {$season}...");
        $this->newLine();

        $results = $gradePlayerProps->execute('basketball_ncaab', $season);
        ...
        $statsByMarket = $gradePlayerProps->getStatsByMarket('basketball_ncaab', $season);
